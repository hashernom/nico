...
// ============================================
// NUEVO: ESTRELLAS Y LUCIÉRNAGAS MODO OSCURO
// ============================================
...
function setupTodoPanel() {
    ...
    function renderTodos() {
        ...
        todos.forEach((todo, index) => {
            ...
            li.innerHTML = `
                <button class="todo-btn ${isChecked}" data-idx="${index}">${checkIcon}</button>
                <span class="todo-text ${doneClass}">${escapeHtml(todo.text)}</span>
                <button class="todo-btn" data-del="${index}"><i class="fas fa-trash-alt"></i></button>
            `;
            list.appendChild(li);
        });
        ...
        list.querySelectorAll('[data-idx]').forEach(btn => {
            btn.addEventListener('click', function() {
                const todos = loadTodos();
                todos[this.dataset.idx].done = !todos[this.dataset.idx].done;
                saveTodos(todos);
                renderTodos();
            });
        });
        ...
        list.querySelectorAll('[data-del]').forEach(btn => {
            btn.addEventListener('click', function() {
                const todos = loadTodos();
                todos.splice(this.dataset.del, 1);
                saveTodos(todos);
                renderTodos();
            });
        });
...
// ============================================
// NUEVO: LIGHTBOX - EXPANDIR IMÁGENES
// ============================================
